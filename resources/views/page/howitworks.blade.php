<!-- Begin Section 3 -->
<section id="media" class="background-apple2 section-padding">
    <div class="container">
        <div class="row">
            <div class="col-xs-12 section-title text-center top-x bottomReveal">
                <h2>How It Works</h2>
                <span class="section-divider mb15"></span>
                <!-- <p class="no-margin">Robo-Advisory Platform</p> -->
            </div>
            <!-- /.column -->
        </div>
        <!-- /.row -->

        <div class="row mt30">
            <div class="col-sm-8  leftReveal">
                <!-- Responsive Video -->
                <h5 class="heading-1 mb20 what">CryptovationX Features</h5>
                <div class="embed-responsive embed-responsive-16by9">
                    <iframe class="embed-responsive-item" src="https://www.youtube.com/embed/AjfWmyxhfcg" allowfullscreen="" width="300px"></iframe>
                </div>
            </div>
            <!-- /.column -->

            <div class="col-sm-4 mt30-xs  rightReveal">

                <!-- Tabs -->
                <h5 class="heading-1 mb20 what">What is CryptovationX?</h5>
                <ul class="nav nav-tabs" role="tablist">



                </ul>
                <!-- Tabs Content -->
                <div class="tab-content tab-default tab-default">

                    <div class="active text-small" id="whitepaper1">
                        <div class="thumbnail no-margin whitepaper">
                            <!-- <img src="images/work/3.jpg" alt="Work"> -->
                            <div class="caption " >
                            <p>White Paper</p> 
                                <a class="btn btn-default" id="White Paper ENG" href=" {{ asset('whitepaper.pdf') }} " target="_blank" role="button">English</a>
                                <a class="btn btn-default" id="White Paper CHI" href=" {{ asset('whitepaper_chi.pdf') }}" role="button">中文</a>
                                <a class="btn btn-default" id="White Paper JPN" role="button" data-toggle="collapse" href="#WhitepaperJapanese" aria-expanded="false" aria-controls="WhitepaperJapanese">日本語</a>
                                <a class="btn btn-default" id="White Paper KOR" href="#" role="button">한국어</a>

                                <div class="collapse jp" id="WhitepaperJapanese">
                                <br>
                                <p>仮想通貨クリプトアセット投資に対するロボ・アドバイザープラットフォーム
                                <br><br>
    及び「全ての人に資産を」イニシアティブ
<br><br>
概要
<br><br>
     クリプトベーションＸは、クリプトベーションにより設立され、AVA Advisory及びAsia Wealth Groupの２社がビジネスパートナーとなっています。「全ての人に資産を」をモットーに、人工知能（AI）の活用により、クリプトアセット投資に最適なロボ・アドバイザリープラットフォームを立ち上げ、クリプト投資家のベストパートナーとなることを目指しています。iOS、Androidのモバイルアプリも開発予定で、６つの基本機能と、Arbi（裁定取引）、Specto（投機的取引）、Broca（ICO）の３つのロボアドバイザーをお使いいただけます。このプラットフォームは、お客様に３つのＡ、Answer（答え）、Assist（サポート）、 Auto-investing（自動投資）をお届けします。
<br><br>
　また、私たちは、投資家の皆様の様々な問題を解決するだけでなく、教育、失業、貧困といった社会経済問題にも目を向けています。「全ての人に資産を」イニシアティブは、①全ての人が、パフォーマンスによる損失補償が可能なアナリスト（予測ができる人）育成、及び社会貢献のためのＣＳＲ活動の実施といった金融教育メカニズム、②年次のコンペを開催し、新たな人材とイノベーションを発掘し、Ｒ＆Ｄ奨励と、エコシステムに関する知識の共有を目的とする、ブロックチェーンと人工知能エコシステムの協働、この２つの戦略を以って、社会に貢献していきます。</p>
                                </div>
                                <!-- <a class="btn btn-default" href="#" role="button">ไทย</a> -->

                            </div>
                        </div>
                    </div>

                </div>
            </div>
            <!-- /.column -->
        </div>
        <!-- /.row -->
</section>